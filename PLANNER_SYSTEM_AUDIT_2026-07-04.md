...
## 2026-07-28 대시보드 배치 후속 감사

### 확인된 현재 구조 문제

- 앱의 데스크톱 화면 밀도는 80~110%가 가능하지만 그리드 드래그 좌표에는 실제 표시 배율이 전달되지 않아 포인터와 카드가 어긋날 수 있습니다.
- 편집기의 위젯 추가 패널이 캔버스 너비를 줄이기 때문에 같은 저장 좌표가 보기 화면과 다른 실제 크기로 렌더링됩니다.
- 편집 시 세로 자동 압축과 즉시 충돌 밀기를 사용해 빈칸과 사용자가 선택한 좌표가 바뀔 수 있습니다.
- 40px 행과 8px 간격의 세로 단위는 48열 가로 단위에 비해 거칠어 세밀한 이동·리사이즈가 어렵습니다.
- 휴대폰은 보기에서 세로 카드로 바뀌지만 편집 정책은 명확히 분리되지 않았고, 위젯마다 내부 반응형 구현 수준이 다릅니다.

### 확정된 목표 구조

- 767px 이하는 세로 목록 순서·노출 편집, 768~1199px은 독립 24열, 1200px 이상은 독립 48열 레이아웃을 사용합니다.
- `DashboardConfig configVersion 1` 아래 PC·태블릿 활성 위젯을 공유하고, 모바일은 그 부분집합만 표시합니다.
- PC·태블릿은 `gridVersion 2`, `ROW_H=16px`, `GAP=8px`을 사용합니다. 기존 세로 좌표·높이를 2배 변환하면 실제 픽셀 배치를 보존하면서 세로 정밀도를 두 배로 높일 수 있습니다.
- 빈칸은 고정하고 점유 위치에서 1초 유지하면 고스트를 표시한 뒤 놓는 시점에만 충돌 위젯을 아래 빈 행으로 이동합니다. 리사이즈는 충돌 경계에서 멈춥니다.
- 편집 패널은 오버레이로 바꾸고 보기·편집 캔버스와 실제 배율 좌표를 동일하게 사용합니다.

### 유지보수·성능 경계

- 배치 버전 변환, 정규화, 충돌·자동 밀기, 태블릿 파생과 모바일 순서·노출 보정은 UI와 분리된 순수 엔진으로 둡니다.
- 위젯 기본·최소 크기와 반응형 규격은 단일 레지스트리로 통합하고, 내부 반응형은 Container Query를 우선합니다.
- 드래그 중 저장과 전체 레이아웃 상태 갱신을 금지하고 포인터 반영을 프레임당 최대 1회로 제한합니다.
- 완료 판정은 포인터 오차 2 CSS px 이하, 입력 반영 p95 34ms 이하, 앱이 만든 50ms 이상 장기 작업 0건과 15개 위젯 반복 조작에서 튐·진동·관찰자 오류가 없는 상태를 기준으로 합니다.
...
| P1 | SC-17 로그인·터치·성능 QA 미완료 | 배율 좌표·공통 캔버스·빈칸/충돌 정책 코드는 구현하고 집중 검사를 통과했지만 실제 계정·터치·15개 위젯 성능은 미확인 | 로그인 Firestore, 태블릿 터치, 1초 유지/취소와 p95·장기 작업 측정 |
...
6. 대시보드 배치 변경은 레거시 픽셀 보존, 기기별 설정, 포인터 정확도와 15개 위젯 성능 측정을 함께 기록합니다.
