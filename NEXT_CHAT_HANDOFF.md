...
_작성일: 2026-06-28_
_갱신일: 2026-07-16 (SC-10 모바일 실기기 피드백 1차 보강 기준)_
...
- 현재 HEAD: `6235ad0 fix: persist edited career event dates`
- 현재 앱 버전: `v0.3.17 UI System Pass`
- 현재 로컬 브랜치 상태: SC-16 신규 사용자 온보딩 코드와 관련 문서가 반영된 상태에서 SC-10 모바일 실기기 피드백 1차 보강을 진행 중. 완료 판정과 커밋/푸시 전 로그인 세션 육안 QA와 사용자 확인 필요
...
- 모바일 로그아웃: 전체 메뉴의 페이지 그리드 아래에 사용자 정보와 별도 로그아웃 버튼을 두고 기존 `saveNow → signOut` 순서를 유지합니다.
- 글쓰기 1차 기준: 767px 이하에서 본문 좌우 12px, 텍스트 입력/선택 16px·44px, textarea 최소 96px, 하단 탭 safe area를 적용합니다.
- 회귀 검사: `npm run check:mobile-ux`로 334/340/345px 안정성, 독립 로그아웃, 입력/터치 CSS 계약을 확인합니다.
