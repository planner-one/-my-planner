...
_작성일: 2026-06-28_
_갱신일: 2026-07-28 (SC-17 운영 배포·자산 확인 완료, 로그인 QA 대기 기준)_
...
운영 링크 Reader는 Cloudflare Worker `https://planner-reader.planner-reader-minsoo.workers.dev`를 사용합니다. 공개 URL 검증, EUC-KR/CP949, HTML 표 셀, meta/JSON-LD, PDF/Jina fallback, 콘텐츠 이미지 우선순위와 선택 OCR을 지원합니다. 2026-07-26에 Innobiz 취업캠프와 하나캐피탈 인크루트 사례를 확인한 뒤 Cloudflare `planner-reader`와 Firebase Hosting을 재배포하고 운영 자산을 검증했습니다. 빠른 메모 전체 목록·작성/편집 모달·목록 입력 보강도 같은 운영 패치 기록에 포함됩니다.

플래너는 Firebase 기반 React/Vite/TypeScript 앱이며 현재 앱 버전은 v0.3.17입니다. SC-16 신규 사용자 온보딩은 코드·자동 검사 후 로그인 QA를 기다리고 있고, SC-10의 동적 viewport 하단 메뉴, 계정별 바로가기, 모바일 테마와 화면 밀도는 운영 배포·로그인 전 자산 확인까지 기록했습니다. 2026-07-28에는 SC-17 대시보드 배치 안정화의 코드 구현·전체 자동 검사·비인증 로컬 브라우저 QA를 마치고 `c84549a`를 GitHub `main`에 push한 뒤 Firebase Hosting 운영 배포와 새 자산 응답을 확인했습니다. 실제 로그인 Firestore 재저장, 태블릿 터치, 1초 유지·취소 실조작과 15개 위젯 성능은 남아 있습니다.

SC-17 배포 중 기존 Hosting rewrite의 `readerPage`·`readerImage` Functions 유효 엔드포인트 경고가 출력됐지만 release는 완료됐습니다. 공개 링크 Reader의 운영 우선 경로는 Cloudflare `planner-reader`이며 Functions rewrite는 fallback 설정으로 유지됩니다.
...
- 현재 로컬 브랜치 상태: `main`의 SC-17 기능 커밋 `c84549a`를 push·Firebase Hosting 배포했습니다. 새 세션에서는 후속 문서 커밋을 포함한 최신 Git 상태와 운영 URL을 먼저 확인합니다.
- 최근 push와 Firebase Hosting 배포는 `planner-one` 계정으로 성공했습니다.

## 2026-07-28 SC-17 대시보드 배치 안정화 진행 기준

- 감사 결과: 앱 배율과 그리드 좌표 보정 분리, 편집 패널로 인한 캔버스 폭 차이, 세로 자동 압축·즉시 충돌 밀기, 40px 행의 거친 단위, 휴대폰 보기·편집 정책 불일치가 어긋남과 버벅임의 주요 원인입니다.
- 기기 정책: 휴대폰 767px 이하는 세로 목록의 위젯 순서·노출만 편집하고, 태블릿 768~1199px은 독립 24열, 데스크톱 1200px 이상은 독립 48열 그리드에서 드래그·리사이즈합니다.
- 활성 위젯: PC·태블릿은 같은 활성 위젯 집합을 공유하고 모바일은 그 부분집합을 표시합니다. 모바일에서 모두 숨겨도 편집 화면에서 복구할 수 있어야 합니다.
- 배치 정책: 빈칸은 자동 압축 없이 유지합니다. 점유 위치에서 1초 머물면 진행 표시와 결과 고스트를 보여주고, 그 상태에서 놓을 때만 겹친 위젯을 아래쪽 가장 가까운 빈 행으로 연쇄 이동합니다. 목표 변경·Escape·pointer cancel은 즉시 취소합니다.
- 크기 정책: 최소 규격 안에서 가로·세로를 자유롭게 조절하고 위젯 내부가 컨테이너에 반응합니다. 리사이즈가 이웃 위젯이나 경계에 닿으면 마지막 유효 크기에서 멈추고 이웃을 밀지 않습니다.
- 저장 구조: `DashboardConfig configVersion 1`에 PC·태블릿 공통 활성 위젯, 두 `gridVersion 2` 레이아웃, 모바일 순서·노출을 둡니다. 기존 `dashboardLayout`·`dashboardActive`는 일괄 삭제하지 않고 새 설정이 없을 때만 픽셀 보존 변환 원본으로 사용합니다.
- 유지보수: 배치 변환·정규화·충돌·자동 밀기·태블릿 파생·모바일 보정은 UI에서 분리된 순수 엔진으로, 기본·최소 크기와 반응형 규격은 단일 위젯 레지스트리로 관리합니다.
- 부드러움: 편집 패널은 캔버스 폭을 바꾸지 않는 오버레이로 열고 실제 표시 배율을 좌표에 반영합니다. 드래그 중 Firestore 저장과 전체 레이아웃 상태 갱신을 막고 포인터 반영을 프레임당 최대 1회로 제한합니다.
- 완료 기준: 15개 위젯 반복 조작에서 위치 튐·크기 진동·관찰자/콘솔 오류가 없고, 포인터 오차 2 CSS px 이하, 입력 반영 p95 34ms 이하, 앱이 만든 50ms 이상 장기 작업 0건을 확인합니다. 집중 검사와 390/767/768/1024/1200/1440px, 80~110% 밀도, 로그인 세션 육안 QA 후에만 완료 상태를 갱신합니다.
- 구현·검증 상태: 버전 설정·마이그레이션·기기별 UI·공통 캔버스·자동 밀기·충돌 리사이즈·저장 잠금·취소 복구·위젯 반응형을 구현했습니다. 온보딩 필드가 없는 기존 사용자의 stale-save 병합이 `onboarding: undefined`를 만들어 Firestore 저장을 거부하던 문제도 병합·트랜잭션 경계 공통 정규화와 레거시 회귀 검사로 차단했습니다. 관련 집중 검사, TypeScript, 빌드와 비인증 로컬 390~1440px·80·90·110% 주요 조작 QA를 통과했습니다.
- 남은 QA: 수정 후 실제 로그인 계정의 대시보드 재저장·새로고침·다중 탭, 저장 실패 복구, 실기기 태블릿 터치, 1초 유지·Escape·pointer cancel, 15개 위젯 p95/장기 작업 측정입니다. 일반 로컬 서버는 `http://127.0.0.1:5173/`에 복구했으며 검증용 임시 화면은 제거했습니다.
...
- 확인된 보완점: 로그인/동적 렌더링 페이지 Reader 제한, 실기기 반응형 QA 필요, 디자인 시스템 불균일, 데이터 내보내기/삭제와 약관/개인정보 문서 필요
...
- 상태: `quickMemos`, `notes`
- 위젯 헤더는 실제 화살표 없이 `+`, `연필`, `보관함` 순서입니다. `+`는 빈 작성 모달, 연필은 관리 모드, 보관함은 노트 페이지의 빠른 메모 수집함을 엽니다.
- 위젯 본문의 한 줄 입력과 최근 4개 제한을 제거하고 미처리 전체 목록을 최신순·두 줄 미리보기로 표시합니다. 일반 모드의 행 선택과 관리 모드의 행별 연필은 같은 편집 모달을 엽니다.
- `1. 항목`과 `- 항목` 다음 Enter는 다음 번호·불릿을 이어 쓰고 빈 목록 행 Enter는 목록을 종료합니다. Shift+Enter는 일반 줄바꿈이며 한글 IME 조합 중 Enter는 자동 변환이나 저장으로 처리하지 않습니다.
- 모달은 Escape·배경 클릭·취소로 닫고 닫힌 뒤 원래 트리거에 포커스를 돌려줍니다. 기존 메모는 모달에서 저장·삭제할 수 있고 삭제는 확인 후 완전 삭제합니다.
- 노트 페이지의 빠른 메모 수집함도 줄바꿈·번호·불릿을 안전하게 표시하고 멀티라인으로 수정합니다.
- 기존 240자 제한과 Todo·정식 노트 전환 및 원본 처리 이력은 유지합니다.
...
- 대시보드 편집 정책과 코드는 SC-17에서 확정·운영 배포했습니다. 태블릿은 별도 24열 드래그·리사이즈, 모바일은 세로 목록 순서·노출 편집만 지원하며 로그인·터치·성능 QA 전에는 실기기 최적화 완료로 안내하지 않습니다.

## 다음 작업 제안 순서

1. 운영 배포된 SC-17을 실제 로그인 계정에서 다시 저장하고 `onboarding: undefined` 오류가 재발하지 않는지 확인
2. 로그인 세션에서 Firestore 저장 실패 복구·새로고침·다중 탭 병합 확인
3. 390/767/768/1024/1200/1440px 실기기/로그인 세션에서 태블릿 터치, 1초 고스트·취소와 15개 위젯 성능 측정
4. SC-10 바로가기·테마·밀도, SC-16 온보딩과 실제 기기 주소창·키보드의 남은 QA 확인
5. 로그인 QA 결과를 코드·자동 검사·운영 배포 상태와 분리 기록하고 필요한 후속 수정은 다시 사용자 승인 후 배포
