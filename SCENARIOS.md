...
- **신청 일정 입력 기준**:
  ...
  - 구분에 따라 신청 마감일·결과 발표일·운영 시작/종료일 중 필요한 입력만 표시
  - 오프라인은 장소/주소를 추가로 표시하고 관련 링크는 진행 방식과 무관하게 항상 입력 가능
  - 운영 기간의 평일 일수를 계산하고 캘린더에 마감·발표·운영 일정을 구분해 표시
  - 결과 발표일은 선택 입력이며, 신규 작성 상태 선택에서는 탈락·취소를 제외
  - 목록 카드에 현재 상태를 명확한 배지와 읽기 쉬운 글씨 크기로 표시
