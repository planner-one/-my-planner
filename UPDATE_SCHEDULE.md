...
_현재 기준: v0.3.17 UI System Pass · v0.3.18 전체는 미릴리즈 · SC-17 운영 배포·자산 확인 완료, 로그인·터치·성능 QA 대기_
...
| v0.3.18 | Planner Flow Clarity | 신규 사용자 온보딩, 플래너 역할 구분, 대시보드 배치 안정화 | 진행 중·미릴리즈, SC-17 운영 배포·로그인 QA 대기 |
...
진행 상태: SC-16 신규 사용자 온보딩의 코드 구현·자동 검사와 SC-10 모바일 탐색·테마·반응형 밀도 보강의 코드·자동 검사·Firebase 운영 배포·자산 확인을 완료했습니다. 현재 앱 버전은 v0.3.17로 유지하며 로그인 세션과 실기기 반응형 QA 전에는 완료 또는 릴리즈로 표시하지 않습니다.
...
- 모바일 1차 보강: 일일 루틴 border-box 크기 안정화, 전체 메뉴 독립 로그아웃·키보드 포커스 관리, 본문/포털 입력 16px·44px, textarea 96px, 본문 12px 여백, 하단 safe area 적용 및 Firebase Hosting 배포
- SC-10 후속 코드 완료: `100dvh`·safe area 하단 메뉴, 홈 고정+계정별 바로가기 네 개와 편집 패널, 모바일 네 테마 선택, 휴대폰·태블릿 100%·웹 자동 90%·웹 직접 설정 80~110%
- SC-10 후속 자동 검사 완료: 반응형 UI 계약, 설정 최신값 병합, 기존 모바일·문서·온보딩·데이터 회귀, 타입·빌드·diff 검증
- SC-10 후속 운영 반영 완료: Firebase Hosting 33개 파일 release complete, 새 JS/CSS hash와 `100dvh`·편집기·테마·자동 밀도 표식 확인, 로그인 전 390/768/1024px scale 1·overflow 0·console error 0 확인. 로그인 후 기능과 실기기 주소창·키보드 QA는 대기
...
### SC-17 대시보드 배치 안정화 — 운영 배포·자산 확인 완료, 로그인 QA 대기

- 확정: 휴대폰 767px 이하는 세로 목록의 순서·노출만 편집
- 확정: 태블릿 768~1199px은 독립 24열, 데스크톱 1200px 이상은 독립 48열 드래그·리사이즈 레이아웃
- 확정: PC·태블릿 활성 위젯은 공유하고 모바일은 그 부분집합을 표시
- 확정: 빈칸은 그대로 유지하고, 점유 위치에서 1초 대기 후 고스트를 확인해 놓을 때만 아래 방향 자동 밀기
- 확정: 위젯은 최소 크기 안에서 자유롭게 조절하고 내부 콘텐츠가 크기에 반응하며, 리사이즈 충돌 시 이웃을 밀지 않고 멈춤
- 코드 완료: `DashboardConfig configVersion 1`, PC·태블릿 `gridVersion 2`, 레거시 픽셀 보존 변환과 모바일 순서·노출 설정
- 코드 완료: 보기·편집 공통 캔버스, 실제 배율 좌표 보정, 드래그 중 저장·전체 재렌더링 차단과 저장 중 편집 잠금
- 유지보수 기준: 마이그레이션·정규화·충돌·자동 밀기·기기별 파생을 순수 엔진으로, 위젯 규격을 공통 레지스트리로 분리
- 검증 완료: 집중 회귀 검사, TypeScript, 프로덕션 빌드와 비인증 로컬 390/767/768/1024/1199/1200/1440px·80/90/110% 주요 조작
- 완료 판정 전 필요: 로그인 Firestore, 실기기 터치, 1초 유지·Escape·pointer cancel, 15개 위젯 p95/장기 작업 측정
- 운영 완료: `c84549a`를 GitHub `main`에 push하고 Firebase Hosting 36개 파일 배포, 운영 HTML·메인/대시보드 자산 HTTP 200 확인
...
| P0 | 대시보드 포인터 좌표·캔버스 폭·충돌 동작 안정화 | 현재 배율과 편집 패널, 자동 압축이 사용자가 지정한 위치와 조작감을 바꿀 수 있음 |
| P1 | 공통 카드 반경/패딩/제목 크기 기준화 | 화면별 인라인 스타일 편차를 줄이기 위함 |
| P1 | 위젯 컨테이너 반응형 규격 통일 | 자유 크기 조절 시 위젯별 잘림과 불필요한 측정·재렌더링을 막기 위함 |
...
| 후속 배포·자산 확인 완료, 로그인 QA 대기 | 실제 기기 반응형 QA 체크리스트, 모바일 입력 1차 보강, 탐색·테마·밀도 후속 보강 | 전체 페이지/위젯, `navigationPreferences`, `displayPreferences` |
...
1. 운영 배포된 SC-17의 실제 로그인 Firestore 재저장과 `undefined` 오류 재발 여부 확인
2. 로그인 Firestore 저장 실패 복구·새로고침·다중 탭 QA
3. 태블릿 터치·1초 유지/취소·15개 위젯 성능과 390~1440px 로그인 세션 육안 QA
4. SC-10 바로가기·테마·밀도와 SC-16 온보딩의 남은 로그인·실기기 QA
5. v0.3.18 Planner Flow Clarity의 목표/프로젝트/작업/오늘 방향 역할 정리
6. v0.3.19 Data Quality & AI Readiness로 상태 이력, 통합 검색, 데이터 내보내기 범위 정리
